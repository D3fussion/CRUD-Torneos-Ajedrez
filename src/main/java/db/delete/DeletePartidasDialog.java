 ...
 * @author Maestro
 */
public class DeletePartidasDialog extends javax.swing.JInternalFrame {
     ...
     */
    public DeletePartidasDialog(java.awt.Frame parent, Database db, JDesktopPane desktopPane) {
    ...
    private void buildCombo() {
        final String sql = "SELECT player_id1, player_id2, tournament_id, match_start_datetime FROM Matches ORDER BY player_id1";
        ...
        try {
            ...
            while (rs.next()) {
                ...
                String torneo_id= rs.getString(3);
                String fecha= rs.getString(4);
                // Agregar nombre del proveedor al combo
                comboProveedor.addItem("Player "+id1+" VS Player "+id2+" | Tournament "+torneo_id+ " | " + fecha);
                values.add(id1+" : "+id2+" : "+torneo_id+" : "+fecha+" ");
                i++;
            }
            if (i == 0) {
                JOptionPane.showInternalMessageDialog(this, "No matches found", "Error", JOptionPane.ERROR_MESSAGE);
                doClose(RET_CANCEL);
            }
    ...
    private void initComponents() {
        ...
        jLabel1.setText("Player 1 ID, Player 2 ID, Tournament ID and Start Date");
...
//            String[] valores =  comboProveedor.getSelectedItem().toString().split(":");
            String[] valores = values.get(comboProveedor.getSelectedIndex()).split(":");
            System.out.println(id);
            String sql = String.format("DELETE FROM Matches WHERE player_id1 = \'%s\' AND player_id2 =  \'%s\' AND tournament_id  = \'%s\' AND match_start_datetime = \'%s\'", valores[0], valores[1], valores[2], valores[3]);
            System.out.println(sql);
            db.update(sql);
            cambios++;
        } catch (SQLException ex) {
            System.out.println(ex.getMessage());
            JOptionPane.showInternalMessageDialog(this, "Error trying to delete match", "Error", JOptionPane.ERROR_MESSAGE);
